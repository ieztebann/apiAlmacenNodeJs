...
const manageVehicle = async (datosVehiculo, idPersona, idUsuario, transaction) => {
    ...
    try {
        ...
        if (!currentVehicle) {
            currentVehicle = await ExternalVehicle.findOne({
                where: {
        ...
        if (!currentVehicle) {
            currentVehicle = await ExternalVehicle.create({"plate":datosVehiculo.plate,"idPersonaPropietario":idPersona,"idUsuarioCre":idUsuario, "observacion":"Vehiculo EDS Facturacion"},{ transaction });
        }
